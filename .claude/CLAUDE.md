...
Indexed by [Repowise](https://repowise.dev). Last indexed: 2026-09-17 (commit 9707ab8). Confidence: 100%.
...
- `src/main/engine/searchEngine.ts` — 6 bug fixes, last fix 6 days ago (bug magnet); 12 commits/90d
- `src/main/index.ts` — 4 bug fixes, last fix 6 days ago (bug magnet); 8 commits/90d
- `src/renderer/src/components/ResultDetailDrawer.tsx` — 2 bug fixes, last fix 6 days ago; 7 commits/90d
- `src/renderer/src/components/ResultsTable.tsx` — 2 bug fixes, last fix 6 days ago; 5 commits/90d
- `src/renderer/src/store.ts` — 2 bug fixes, last fix 3 weeks ago; 7 commits/90d

### Code health
Three co-equal signals: code health 8.04/10 avg (Good), hotspot health 6.34/10 (improving), worst `src/main/engine/searchRun.ts` at 5.55/10 · maintainability 8.66/10 · performance risk 17 open static I/O-in-loop / N+1 findings. Detail: `get_health()`.

Critical files:
- `src/renderer/src/store.ts` — change entropy — impact −1.3
- `src/main/index.ts` — change entropy — impact −1.0
- `src/main/engine/searchEngine.ts` — change entropy — impact −0.9
- `src/main/engine/searchEngine.ts` — churn risk — impact −0.7
- `src/main/index.ts` — prior defect — impact −0.7
